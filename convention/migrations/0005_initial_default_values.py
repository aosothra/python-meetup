...
def test_values(apps, schema_editor):
    ...
    Event.objects.create(
        title = "Лучшее мероприятие для программистов",
        starting_date = datetime.datetime(2022, 8, 9),
        ending_date = datetime.datetime(2022, 8, 10)
    )

    event = Event.objects.get(title="Лучшее мероприятие для программистов")

    Flow.objects.create(
        title = "Вступительные мероприятия",
        event = event)
    ...
    Flow.objects.create(
        title = "Заключительные мероприятия",
        event = event)
